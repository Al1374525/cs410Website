<!DOCTYPE html>
<html lang="en" dir="ltr">
    <head>
        <meta charset ="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title> Gamer Connection | Problem</title>
        <link rel="stylesheet" href="style.css">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.2/css/all.min.css"/>
    </head>
<body>
    <?php include 'nav.php'; ?>
    
    <main class="content-wrapper">
        <h1 class="page-title">The Problem</h1>
        <section class="problem-description">
            <p>
                The challenge is the isolation and difficulty gamers face in discovering new titles while lacking a dedicated platform to connect with like-minded players. 
                Many gamers find it hard to explore new games and build a community around their gaming interests, leading to a sense of disconnection in an increasingly vast and diverse gaming landscape.
            </p>
        </section>
    </main>
</body>
</html>